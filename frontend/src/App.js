...
const countHandler = input => {
    ...
    if(input === 'plus') {
        setCount(count.map(item => item.id === id ? 
          { ...item, date: dateString, count: item.count + 1, beers: [...item.beers, time] } : item))
    } else if(input === 'minus' && counter > 0) {
        const beers = count[count.length -1].beers
        beers.length = beers.length -1
        setCount(count.map(item => item.id === id && item.count > 0 ? 
...
const yellow = "#F1B10F"
  ...
  return (
    ...
    { config && <Config bloodAlcoholContent={ bloodAlcoholContent } configHandler={ configHandler }  menuItem={ menuItem } /> }
    { menu && <Menu count={ count } deleteItem={ deleteItem } /> }

     <Wave fill={ mainScreen ? 0 : fill } />

    { mainScreen || <Counter count={ count } countHandler={ countHandler } bloodAlcoholContent={ bloodAlcoholContent } /> }
